...
use hal::gpio::{FunctionPio0,FunctionPio1, FunctionPio2, Pin};
use hal::pio::PIOExt;
use hal::Sio;
...
fn main() -> ! {
    ...
    let _pin0 = pins.gpio0.into_pull_down_input();
    ...
    let (mut sm_pio0, _, _) = hal::pio::PIOBuilder::from_installed_program(installed0)
        .set_pins(led0.id().num, 1)
        .clock_divisor_fixed_point(int, frac)
        .build(sm0_pio0);
    sm_pio0.set_pindirs([(led0.id().num, hal::pio::PinDir::Output)]);
    sm_pio0.start();

    let (mut pio1, sm0_pio1, _, _, _) = pac.PIO1.split(&mut pac.RESETS);
    let installed1 = pio1.install(&program1.program).unwrap();
    let (mut sm_pio1, _, _) = hal::pio::PIOBuilder::from_installed_program(installed1)
        .set_pins(led1.id().num, 1)
        .clock_divisor_fixed_point(int, frac)
        .build(sm0_pio1);
    sm_pio1.set_pindirs([(led1.id().num, hal::pio::PinDir::Output)]);
    sm_pio1.start();

    let (mut pio2, sm0_pio2, _, _, _) = pac.PIO2.split(&mut pac.RESETS);
    let installed2 = pio2.install(&program2.program).unwrap();
    let (mut sm_pio2, _, _) = hal::pio::PIOBuilder::from_installed_program(installed2)
        .out_pins(led2.id().num, 1) // set -> out
        .clock_divisor_fixed_point(int, frac)
        .build(sm0_pio2);
    sm_pio2.set_pindirs([(led2.id().num, hal::pio::PinDir::Output)]);
    sm_pio2.start();
